...
public class ZEDBodyTrackingManager : MonoBehaviour
{
    ...
    [Space(5)]
    [Header("------Instantiate Particle ------")]
    public GameObject skeletonPtc;
    public GameObject skinnerPtc;
    ...
    IEnumerator InstanciateAvatarWithDelay(float delay, DetectedBody dbody)
    {
        ...
        handler.rootXmultiply = rootXmultiply;
    ...
    void instantiateParticle(SkeletonHandler handler)
    {
        GameObject skingo = Instantiate(skinnerPtc, Vector3.zero, Quaternion.identity); ;
        GameObject skelgo = Instantiate(skeletonPtc, Vector3.zero, Quaternion.identity); ;
        Dance_AvatarContorller avatarCtrler = handler.humanoid.GetComponent<Dance_AvatarContorller>();
        if (avatarCtrler != null)
        {
            Skeleton skeleton = skelgo.GetComponent<Skeleton>();
            SkinnerGlitch skinGlitch = skingo.transform.GetChild(1).GetComponent<SkinnerGlitch>();
            SkinnerParticle skinnerParticle1 = skingo.transform.GetChild(2).GetComponent<SkinnerParticle>();
            SkinnerParticle skinnerParticle2 = skingo.transform.GetChild(3).GetComponent<SkinnerParticle>();
            SkinnerTrail skinnerTrail = skingo.transform.GetChild(4).GetComponent<SkinnerTrail>();

            avatarCtrler.slekVfx = skeleton;
            avatarCtrler.skinnerGlitch = skinGlitch;
            avatarCtrler.skinnerParticle1 = skinnerParticle1;
            avatarCtrler.skinnerParticle2 = skinnerParticle2;
            avatarCtrler.skinnerTrail = skinnerTrail;

            skinGlitch.source = avatarCtrler.skinSource;
            skinnerParticle1.source = avatarCtrler.skinSource;
            skinnerParticle2.source = avatarCtrler.skinSource;
            skinnerTrail.source = avatarCtrler.skinSource;
            skeleton._sourceAnimator = avatarCtrler.GetComponent<Animator>();
        }
    }
    ...
    private void UpdateSkeletonData(BodyTrackingFrame dframe)
    {
        ...
        foreach (int index in remainingKeyList)
        {
            ...
            handler.GetAnimator().gameObject.GetComponent<Dance_AvatarContorller>().LimitSensingZed(false);
            StartCoroutine(DelayDestroy(2, handler));
            avatarControlList.Remove(index);
        }
    }

    IEnumerator DelayDestroy(float time, SkeletonHandler handelr)
    {
